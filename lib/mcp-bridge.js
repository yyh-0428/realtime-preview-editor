...
(function (global) {
    ...
    MCPBridge._connectSSE = function (conn) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            // 通用 message 事件：endpoint 发现 + JSON-RPC 响应分发
            es.addEventListener('message', function (evt) {
                // 尚未拿到 endpoint 时，优先尝试 endpoint 发现
                if (!conn.postEndpoint) {
                    ...
                    } catch (e) { /* 非 endpoint 消息，落到下面当响应处理 */ }
                }
                // endpoint 就绪后（或非 endpoint 消息），作为 JSON-RPC 响应处理
                self._handleSSEMessage(conn, evt.data);
            });
