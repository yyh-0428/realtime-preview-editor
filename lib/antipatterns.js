...
(function (global) {
     ...
     *  返回与原 code 等长的掩码字符串：非 CSS 部分替换为空格（保留换行），
     *  CSS 部分（<style> 块内容 + inline style 值）保持原样。
     *  这样 regex 匹配的 index 与原文一致，lineOf() 能返回正确行号。 */
    function extractStyleFromHtml(code) {
        ...
        while ((m = re.exec(code)) !== null) {
            var contentStart = m.index + m[1].length;
            var contentEnd = contentStart + m[2].length;
        ...
        while ((m = styleAttrRe.exec(code)) !== null) {
            var attrStart = m.index + m[1].length;
            var attrEnd = attrStart + m[2].length;
            for (i = attrStart; i < attrEnd; i++) {
                chars[i] = code.charAt(i);
            }
        }
        return chars.join('');
    ...
    function ruleNestedCards(code) {
        var issues = [];
        var openRe = /<\w+[^>]*class\s*=\s*["'](?:[^"']*\s)?card(?:\s[^"']*)?["'][^>]*>/gi;
        // 内层用非全局正则做存在性判断，避免 lastIndex 干扰
        var innerRe = /<\w+[^>]*class\s*=\s*["'](?:[^"']*\s)?card(?:\s[^"']*)?["'][^>]*>/i;
        var m;
    ...
    function ruleIconTileStack(code) {
        ...
        // 补充：检测 inline style 中固定尺寸 + border-radius + background 的图标容器（支持任意属性顺序）
        var inlineRe = /style\s*=\s*["']([^"']+)["']/gi;
        while ((m = inlineRe.exec(code)) !== null) {
            var styleVal = m[1];
            if (/width\s*:\s*\d+px/i.test(styleVal) &&
                /height\s*:\s*\d+px/i.test(styleVal) &&
                /border-radius/i.test(styleVal) &&
                /background/i.test(styleVal)) {
    ...
    function ruleBounceEasing(code, lang) {
        ...
        if (lang === 'javascript' || lang === 'html') {
            var jsRe = /\bease\s*[:.=\[]?\s*["']?\s*(?:bounce|elastic|back)\b/gi;
    ...
    function ruleGrayOnColor(code) {
        ...
        while ((m = blockRe.exec(code)) !== null) {
            ...
            var declRe = /(background(?:-color)?|color)\s*:\s*(#[0-9a-f]{3,6}|[a-z]+)/gi;
            var dm;
